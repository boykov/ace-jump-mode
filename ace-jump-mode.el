...
buffer."
  ...
  (cond
   ...
   ((memq (ace-jump-char-category head-char)
    ...
    (ace-jump-do (concat "\\<" (eab/or-char (make-string 1 head-char)))))
   ((eq (ace-jump-char-category head-char)
    ...
    (setq ace-jump-query-char head-char)
    (setq ace-jump-current-mode 'ace-jump-char-mode)
    (ace-jump-do (eab/or-char (make-string 1 head-char))))
   (t
    (error "[AceJump] Non-printable character"))))
...
;;	   \___|\__,_|_.__/  |___/ |___/\__|\__,_|_| |_|
;;
;;

(defun eab/insert-or-char (c)
  (interactive)
   (let ((method current-input-method)
	(or-mac (read-kbd-macro c)))
...
(defmacro eab/with-kbd-output-to-string (&rest body)
  `(progn
    (switch-to-buffer (generate-new-buffer-name "*temp*"))
    (progn ,@body)
    (let ((tmp (buffer-substring (point-min) (point-max))))
      (kill-buffer)
      tmp)))

(defvar eab/char-en-ru)

(setq eab/char-en-ru (make-hash-table :test 'equal))
	 ...
	 (remove-if
	  (lambda (x)
	    (or
	     ...
	     (eq x (string-to-char "|"))
	     (eq x (string-to-char "/"))))
	  (number-sequence 34 125)))
	(buffer-string)) "" 't))) " " 't)))

(setq eab/char-en-ru-special '(";" "ж"
			       ":" "Ж"
			       "." "ю"
			       ">" "Ю"
			       "," "б"
